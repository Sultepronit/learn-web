...
const initServer = http.createServer((req, res) => {
	res.writeHead(200, {'Content-Type': 'text/html'});
	const jscript = '<script>var strWindowFeatures = "location=yes,height=970,width=520,scrollbars=yes,status=yes";' +
	'var URL = "http://localhost:8989";' + 
	'var win = window.open(URL, "_blank", strWindowFeatures);' +
	'close();</script>';
	res.write(`<html><body><p>Hello there</p>${jscript}</body></html>`);
	res.end();
});
initServer.listen(1199);

const spawn = require('child_process').spawn;
// open default browser
spawn('open', ['http://localhost:1199']);
console.log('running on http://localhost:1199');

/*
var strWindowFeatures = "location=yes,height=570,width=520,scrollbars=yes,status=yes";
var URL = "http://localhost:8990/pitch";
var win = window.open(URL, "_blank", strWindowFeatures);
close();
*/
